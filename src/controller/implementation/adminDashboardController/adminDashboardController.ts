...
class AdminDashboardController implements IAdminDashboardController {
    ...
    async getOverView(req: Request, res: Response): Promise<Response> {
        try {
            ...
            return res.status(StatusCode.OK)
                .json(generateSuccessResponse("dashboard data fetthced suvvessfully", overview))
            ...
            return res.status(error instanceof CustomError ? error.statusCode : StatusCode.INTERNAL_SERVER_ERROR)
                .json(generateErrorResponse(error instanceof CustomError ? error.message : ERROR_MESSAGES.INTERNAL_SERVER_ERROR))
    ...
    async getRevenueTrend(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!startDate || !endDate || !interval) {
                throw new CustomError("Missing required query parameters", StatusCode.INTERNAL_SERVER_ERROR)
            }
            ...
            return res.status(StatusCode.OK).json(generateSuccessResponse("data fetched succefully", result))
    ...
    async getPlanDistribution(req: Request, res: Response): Promise<Response> {
        try {
            ...
            return res.status(StatusCode.OK).json(generateSuccessResponse("Plan distribution fetched successfully", result))
        } catch (error) {
    ...
    async downloadSubscriptionReport(req: Request, res: Response): Promise<Response> {
        try {

            const { startDate, endDate, format } = req.query;

            if (!startDate || !endDate || !format) {
                throw new CustomError("Missing required parameters", StatusCode.BAD_REQUEST);
            }

            const result = await this._adminDashboardService.generateSubscriptionReport(
                new Date(startDate as string),
                new Date(endDate as string),
                format as "pdf" | "excel"
            )
    ...
    async getDoctorAnalyticsSummary(req: Request, res: Response): Promise<Response> {
        try {

            const result = await this._adminDashboardService.getDoctorAnalyticsSummary()

            if (!result) {
                throw new CustomError("data not found", StatusCode.BAD_REQUEST)
            }

            return res.status(StatusCode.OK).json(generateSuccessResponse("Doctor analytics summary fetched", result))
        } catch (error) {
    ...
    async getDoctorRevenueTrend(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!startDate || !endDate || !["day", "month"].includes(interval as string)) {
                throw new CustomError("Invalid or missing query parameters", StatusCode.BAD_REQUEST);
            }
            ...
            return res.status(StatusCode.OK).json(generateSuccessResponse("Revenue trend fetched", result))
    ...
    async getServiceRevenue(req: Request, res: Response): Promise<Response> {
        try {
            ...
            return res.status(StatusCode.OK).json(generateSuccessResponse("Service revenue fetched", result))
    ...
    async getTopPerformingDoctors(req: Request, res: Response): Promise<Response> {
        try {

            const result = await this._adminDashboardService.getTopPerformingDoctors()
