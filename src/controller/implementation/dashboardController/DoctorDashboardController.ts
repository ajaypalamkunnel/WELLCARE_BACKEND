...
class DoctorDashboardController implements IDoctorDashboardController {
    ...
    async getAppointmentTrend(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!doctorId) {
                throw new CustomError("Unauthorized doctor access", StatusCode.FORBIDDEN);
            }

            const { startDate, endDate, interval } = req.query;

            if (!startDate || !endDate || !interval) {
                throw new CustomError("Missing required query parameters", StatusCode.BAD_REQUEST);
            }

            const trendData = await this._doctorDashboardService.getAppointmentTrendData(
                new Types.ObjectId(doctorId),
            ...
            return res.status(StatusCode.OK).json(
                generateSuccessResponse("Appointment trend data fetched successfully", trendData)
            );
    ...
    async getRevenueTrend(req: Request, res: Response): Promise<Response> {
        try {
            ...
            return res.status(StatusCode.OK).json(
                generateSuccessResponse("Revenue trend data fetched", result)
            );
    ...
    async generateDoctorReport(req: Request, res: Response): Promise<Response> {
        try {
            ...
            return res.status(StatusCode.OK).json(
                generateSuccessResponse("Report generated successfully", { downloadUrl: url })
            );
