...
class SubscriptionController implements ISubscriptionController {
    ...
    async getAllSubscriptionPlans(req: Request, res: Response): Promise<Response> {
        try {
            ...
            return res.status(StatusCode.OK).json(generateSuccessResponse("Subscription plans fetched successfully", plans))

        } catch (error) {

            console.error("Error fetching subscription plans:", error);

            return res.status(error instanceof Error ? StatusCode.INTERNAL_SERVER_ERROR : StatusCode.BAD_REQUEST)
                .json(generateErrorResponse(error instanceof Error ? error.message : "Internal Server Error"));
        }
