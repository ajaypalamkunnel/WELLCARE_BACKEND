...
class PrescriptionController implements IPrescriptionController{
    ...
    async submitPrescription(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            const prescription = await this._prescriptionService.submitPrescription(req.body)

            if(!prescription){
                throw new CustomError("Prescription creation failed",StatusCode.BAD_REQUEST)
            }


            return res.status(StatusCode.ACCEPTED)
                .json(generateSuccessResponse("Prescription added successfully",prescription))
    ...
    async getPrescription(req: Request, res: Response): Promise<Response> {
       try {
        ...
        const prescription = await this._prescriptionService.getPrescriptionByAppointment(appoinmentId)
        ...
        return res.status(StatusCode.OK)
            .json(generateSuccessResponse("Prescription fetched successfully",prescription))
       } catch (error) {

        return res.status(error instanceof CustomError ? error.statusCode : StatusCode.INTERNAL_SERVER_ERROR)
        .json(generateErrorResponse(error instanceof CustomError ? error.message : "Unexpected error occurs"))
