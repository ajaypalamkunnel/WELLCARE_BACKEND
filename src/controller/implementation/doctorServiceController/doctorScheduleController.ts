...
class DoctorScheduleController implements IDoctorScheduleController {
    ...
    async validateSchedule(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (isNaN(startDateTime.getTime()) || isNaN(endDateTime.getTime())) {
                return res.status(StatusCode.BAD_REQUEST).json({ success: false, message: "Invalid date or time format." });
            }
            ...
            const validationResult = await this._doctorScheduleService.validateSchedule(
                doctorId,
                service,
                new Date(date),
                startDateTime,
    ...
    async generateSlot(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!doctorId || !service || !date || !start_time || !end_time || !duration) {
                throw new CustomError("Missing required fields", StatusCode.BAD_REQUEST);
            }


            const generatedSlots = await this._doctorScheduleService.validateAndGenerateSlots(
                doctorId, service, new Date(date), start_time, end_time, duration
            )

            return res.status(StatusCode.OK).json(generateSuccessResponse("slots generated successfully", generatedSlots))
    ...
    async createSchedule(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const { doctorId, serviceId, date, start_time, end_time, duration, slots } = req.body;

            console.log("create schedule controller ===> ",req.body);
            

            if (!doctorId || !serviceId || !date || !start_time || !end_time || !duration || !slots) {
            ...
            startDateTime.setHours(startDateTime.getHours() + 5, startDateTime.getMinutes() + 30); // Convert to IST

            const endDateTime = new Date(`${date}T${end_time}:00`);
            endDateTime.setHours(endDateTime.getHours() + 5, endDateTime.getMinutes() + 30); // Convert to IST
        ...
        const formattedSlots:ISlot[] = slots.map((slot: ISlot) => ({
            ...
            start_time: new Date(new Date(slot.start_time).getTime() + (5.5 * 60 * 60 * 1000)), // Convert to IST
            end_time: new Date(new Date(slot.end_time).getTime() + (5.5 * 60 * 60 * 1000)), // Convert to IST
   ...
   async listSchedules(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!doctorId) {
                return res.status(StatusCode.BAD_REQUEST).json({ success: false, message: "Doctor ID is required." });
            }
