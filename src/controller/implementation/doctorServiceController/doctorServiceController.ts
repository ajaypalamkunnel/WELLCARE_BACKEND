...
class DoctorServiceController implements IDoctorServiceController {
    ...
    async createDoctorService(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            return res.status(StatusCode.CREATED).json(generateSuccessResponse("New Service created successfully", service))
    ...
    async doctorServiceUpdate(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const updatedService = await this._doctorServiceService.updateDoctorService(updateData._id,updateData.doctorId,updateData)


            return res.status(StatusCode.OK).json(generateSuccessResponse("Doctor service updated",updatedService))
