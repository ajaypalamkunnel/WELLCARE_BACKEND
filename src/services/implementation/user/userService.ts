...
class UserService implements IUserService {
    ...
    async changePassword(
        ...
        try {
            ...
            const updated = await this._userRepository.updatePassword(
                userId,
                hashedPassword
            );
    ...
    async completeUserRegistration(
        ...
        try {
            ...
            const completePersonalInfo: IUser["personalInfo"] = {
                ...
                blood_group:
                    personalInfo.blood_group ?? user.personalInfo.blood_group ?? "",
                allergies: personalInfo.allergies ?? user.personalInfo.allergies ?? "",
                chronic_disease:
                    personalInfo.chronic_disease ??
                    user.personalInfo.chronic_disease ??
                    "",
            };
    ...
    async fetchScheduleByDoctorAndDate(
        doctorId: string,
        date: string
        ...
        try {
            ...
            const schedules = await this._userRepository.fetchDoctorDaySchedule(
                doctorId,
                dateObj
            );
            ...
            throw new CustomError(
                "Error in fetching doctor schedule",
                StatusCode.INTERNAL_SERVER_ERROR
            );
        }
    }
    ...
    async fetchNotifications(userId: string): Promise<INotification[]> {
        try {
            const notifications = await this._userRepository.getAllNotifications(
                userId
            );
