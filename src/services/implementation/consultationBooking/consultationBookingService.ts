...
class ConsultationBookingService implements IConsultationBookingService {
    ...
    constructor(
        ...
        doctorWalletRepository: IDoctorWalletRepository
    ) {
    ...
    async getBookingDetails(
        ...
        try {
            const booking =
                await this._consultationBookingRepository.getABookingDetails(bookingId);
            ...
            if (
                bookingObj.doctorScheduleId &&
                bookingObj.doctorScheduleId.availability
            ) {
    ...
    async initiateBooking(data: {
        ...
        try {
            ...
            if (!schedule) {
                throw new CustomError(
                    "Slot is either booked or invalid",
                    StatusCode.CONFLICT
                );
            }

            const slot = schedule.availability.find(
                (s: ISlot) => s.slot_id.toString() === slotId
            );
            ...
            const marked = await this._doctorScheduleRepository.markSlotAsPending(
                doctorScheduleId,
            ...
            if (!marked) {
                throw new CustomError(
                    "Slot is already booked or being processed",
                    StatusCode.CONFLICT
                );
            }
    ...
    async verifyAndBook(data: {
        ...
        try {
            ...
            const existingPayment =
                await this._consultationBookingRepository.getPaymentByOrderId(
                    razorpay_order_id
                );
            ...
            const schedule = await this._doctorScheduleRepository.findPendingSlot(
                doctorScheduleId,
                slotId
            );
            ...
            if (!schedule) {
                ...
                throw new CustomError(
                    "Slot is already booked or invalid",
                    StatusCode.CONFLICT
                );
            }
            ...
            await sendNotificationToUser(
                ...
                `An appointment has been booked by a patient on ${dayjs(
                    schedule.date
                ).format("DD MMM YYYY")}`
            );
            ...
            await sendNotificationToUser(
                ...
                "user",
                "Booking Confirmed",
                `Your appointment with the doctor is confirmed for ${dayjs(
                    schedule.date
                ).format("DD MMM YYYY")}`,
    ...
    async getUserAppointments(
        ...
        try {
            ...
            const results =
                await this._consultationBookingRepository.findAppointmentsByPatientAndStatus(
                    patientId,
                    statusList
                );
    ...
    async getAppoinmentDetailsById(
        ...
        try {
            ...
            const appointment =
                await this._consultationBookingRepository.findAppoinmentDetailById(
                    appoinmentId,
                    patientId
                );
    ...
    async cancelAppointment(
        ...
        try {
            ...
            const appointment =
                await this._consultationBookingRepository.findByIdAndPatient(
                    appointmentId,
                    patientId
                );
            ...
            const isEligibleForRefund =
                slotStartTime.getTime() - now.getTime() >= 4 * 60 * 60 * 1000;

            const service = await this._doctorServiceRepository.findById(
            ...
            await this._consultationBookingRepository.updateAppointmentCancellation(
                appointment._id,
                {
                    status: "cancelled",
                    cancellation: {
    ...
    async findAppointmentsForDoctor(
        ...
        try {
            ...
            const appointment =
                await this._consultationBookingRepository.findAppointmentsForDoctor(
                    doctorId,
                    filters
                );
    ...
    async getAppointmentDetailForDoctor(
        ...
        try {
            ...
            const detail =
                await this._consultationBookingRepository.findAppointmentDetailForDoctor(
                    appointmentObjectId,
                    doctorId
                );
