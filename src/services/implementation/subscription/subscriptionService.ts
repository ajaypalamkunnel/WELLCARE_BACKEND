...
class SubscriptionService implements ISubscriptionService {
    ...
    async createsubscriptionPlan(data: ISubscription): Promise<ISubscription> {
        try {
            ...
            const { planName, duration, price, discount, finalPrice, serviceLimit, status, features } = data;

            if (!planName || !duration || !price || !discount || !finalPrice || !serviceLimit || !status || !features || features.length === 0) {

                throw new CustomError("Missing required fields", 400);
            }

            const existingPlan = await this._subscriptionRepository.findSubscriptionPlanByName(planName);
            if (existingPlan) {
                throw new CustomError("Plan with this name already exists", 400);
            }

            if (finalPrice > price) {
                throw new CustomError("Final price cannot be greater than the original price", 400);
            }

            const newSubscriptionPlan = await this._subscriptionRepository.create(data);

            if (!newSubscriptionPlan) {
                throw new CustomError("Failed to create subscription plan", StatusCode.INTERNAL_SERVER_ERROR)
            }
    ...
    async getSubscriptionPlans(): Promise<ISubscription[]> {
        try {
            ...
            if (!subscriptionPlans || subscriptionPlans.length === 0) {
                throw new CustomError("No subscription plans found", StatusCode.NOT_FOUND);
            }
            ...
            throw new CustomError("Failed to retrieve subscriptions", StatusCode.INTERNAL_SERVER_ERROR)
    ...
    async toggleSubscriptionStatus(planId: string): Promise<ISubscription> {
        try {
            ...
            console.error(`Subscription plan ${plan.status ? "unblocked" : "blocked"}: ${planId}`);
            return plan;
            ...
            console.error(`Error updating subscription plan status: ${(error as Error).message}`);
            if (error instanceof CustomError) {
                throw error
            }
            throw new CustomError("Failed to update status", StatusCode.INTERNAL_SERVER_ERROR)
        }
    }


    async updateSubscriptionPlan(planId: string, updatedData: Partial<ISubscription>): Promise<ISubscription> {
        ...
        try {
            ...
            const updatedPlanData = {
                ...
                finalPrice:
                    updatedData.discount && updatedData.price
                        ? updatedData.discount.type === "amount"
                            ...
                            : updatedData.price - (updatedData.price * updatedData.discount.amount) / 100
                ...
                features: Array.isArray(updatedData.features)
                    ? updatedData.features.every(f => typeof f === 'object' && 'name' in f)
                        ? (updatedData.features as { name: string }[]).map(feature => feature.name)
            ...
            const updatedPlan = await this._subscriptionRepository.update(planId, updatedPlanData)
