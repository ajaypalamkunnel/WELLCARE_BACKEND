...
class DoctorServiceService implements IDoctorServiceService {
    ...
    constructor(doctorServiceRepository: IDoctorServiceRepository, doctorRepository: IDoctorRepository, doctorSubscriptionRepository: IDoctorSubscriptionsRepository) {
    ...
    async createService(data: IDoctorService): Promise<IDoctorService> {
        try {
            ...
            if (!doctor.isSubscribed || !doctor.currentSubscriptionId) {
                throw new CustomError("Doctor is not subscribed to a plan", StatusCode.FORBIDDEN);
            }
   ...
   async getServicesByDoctor(doctorId: string): Promise<IDoctorService[]> {
        try {
            ...
            const services = await this._doctorServiceRepository.findAllServiceByDoctorId(doctorId)
    ...
    async updateDoctorService(serviceId: string, doctorId: string, updateData: Partial<IDoctorService>): Promise<IDoctorService> {
        try {
            ...
            if(service.doctorId.toString() !== doctorId){
                throw new CustomError("Unauthorized to update this service",StatusCode.UNAUTHORIZED)
            }


            const updatedService = await this._doctorServiceRepository.updateService(serviceId,updateData)

            if(!updatedService){
                throw new CustomError("Failed to update doctor service",StatusCode.INTERNAL_SERVER_ERROR)
            }
