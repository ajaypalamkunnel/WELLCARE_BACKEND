...
class AdminDashboardService implements IAdminDashboardService {
    ...
    async getOverview(): Promise<SubscriptionOverviewDTO> {
        try {
            ...
            console.error(
                "Error in SubscriptionAnalyticsService.getOverview:",
                error
            );
            throw new CustomError(
                "Failed to fetch subscription analytics overview",
    ...
    async getRevenueTrend(
        ...
        try {
            return await this._adminDashboardRepo.getRevenueTrend(
                startDate,
                endDate,
                interval
            );
    ...
    async getPlanDistribution(
        ...
        try {
            return await this._adminDashboardRepo.getPlanDistribution(
                startDate,
                endDate
            );
        } catch (error) {
            if (error instanceof CustomError) {
                ...
                throw new CustomError(
                    "Service error: Failed to fetch plan distribution",
                    StatusCode.INTERNAL_SERVER_ERROR
                );
            }
        }
    ...
    async generateSubscriptionReport(
        ...
        try {
            const data = await this._adminDashboardRepo.getSubscriptionsForReport(
                startDate,
                endDate
            );
    ...
    async getDoctorAnalyticsSummary(): Promise<DoctorAnalyticsSummaryDTO[]> {
        try {
            ...
            throw new CustomError(
                "Failed to fetch doctor analytics summary",
                StatusCode.INTERNAL_SERVER_ERROR
            );
        }
    }
    ...
    async getDoctorRevenueTrend(
        ...
        try {
            const result = await this._adminDashboardRepo.getDoctorRevenueTrend(
                startDate,
                endDate,
                interval
            );

            const preparedData = result.map(
                (entry): RevenueDoctorTrendDTO => ({
                    label: entry.label,
    ...
    async getServiceRevenue(): Promise<ServiceRevenueDTO[]> {
        try {
            ...
            throw new CustomError(
                "Failed to fetch service revenue",
                StatusCode.INTERNAL_SERVER_ERROR
            );
        }
    }
    ...
    async getTopPerformingDoctors(): Promise<TopDoctorDTO[]> {
        try {
            ...
            throw new CustomError(
                "Failed to fetch top-performing doctors",
                StatusCode.INTERNAL_SERVER_ERROR
            );
        }
    }
