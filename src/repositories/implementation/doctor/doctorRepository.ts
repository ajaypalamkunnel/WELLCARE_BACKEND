...
class DoctorRepository
    ...
    async updateDoctorRefreshToken(
        id: string,
        refreshToken: string
    ...
    async updateDoctorVerification(
        ...
        return await Doctor.findByIdAndUpdate(doctorId, updateFields, {
            new: true,
        }).select("-password -refreshToken");
    }
    ...
    async findDoctorsWithFilters(
        ...
        try {
            ...
            const doctors = await Doctor.find(filters)
                ...
                .select(
                    "-password -refreshToken -subscriptionExpiryDate -licenseDocument -IDProofDocument"
                )
                .sort(sortOption)
                .skip(skip)
                .limit(limit);
    ...
    async findDoctorByIdAndGetSubscriptionDetails(
        doctorId: string
    ): Promise<IDoctor | null> {
        try {
            const doctorSubscriptionDetails = await Doctor.findById(doctorId)
    ...
    async addCertification(
        ...
        try {
            ...
            const updateCertificate = await Doctor.findByIdAndUpdate(
                doctorId,
                {
                    $push: {
    ...
    async editCertification(
        ...
        try {
            ...
            if (!updatedCertification) {
                throw new CustomError(
                    "Updated certification not found",
                    StatusCode.NOT_FOUND
                );
            }
